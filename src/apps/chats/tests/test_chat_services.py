...
class ChatServiceTestCase(TestCase):
    def setUp(self):
        self.user1 = MyUser.objects.create_user(first_name='user1',last_name='user1', email='test1@example.com', password='pass')
        self.user2 = MyUser.objects.create_user(first_name='user2',last_name='user2', email='test2@example.com', password='pass')
    ...
    def test_create_conversation_success(self):
        conv_id = self.service.create_conversation([self.user1.id, self.user2.id])
        conv = Conversation.objects.get(id=conv_id)
        self.assertEqual(set(conv.participants.all()), {self.user1, self.user2})

    def test_create_conversation_no_participants(self):
        with self.assertRaises(ValidationError):
            self.service.create_conversation([9999])

    def test_join_leave_conversation(self):
        conv_id = self.service.create_conversation([self.user1.id])
        self.service.join_conversation(conv_id, self.user2.id)
        conv = Conversation.objects.get(id=conv_id)
        self.assertIn(self.user2, conv.participants.all())
        self.mock_redis_consumer_repo.add_to_set.assert_called_with(f'active_users:{conv_id}', str(self.user2.id))

        self.service.leave_conversation(conv_id, self.user1.id)
        conv.refresh_from_db()
        self.assertNotIn(self.user1, conv.participants.all())
        self.mock_redis_consumer_repo.remove_from_set.assert_called_with(f'active_users:{conv_id}', str(self.user1.id))
    ...
    def test_get_active_users_returns_users(self):
        conv_id = self.service.create_conversation([self.user1.id, self.user2.id])
        self.mock_redis_consumer_repo.get_set_members.return_value = [str(self.user1.id), str(self.user2.id)]
        ...
        self.mock_redis_consumer_repo.get_set_members.assert_called_once_with(f'active_users:{conv_id}')
